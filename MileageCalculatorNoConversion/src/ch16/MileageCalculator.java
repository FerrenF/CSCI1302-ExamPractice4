...
public class MileageCalculator extends Application {
	...
	private void changeLabels() {
		...
		//Get the units that we are converting from by reading the text of the distance label.
		String fromUnits = lblDistance.getText();
		
		//Get the units that we are converting to by reading the text of the combo box (Using our shortened method.)
	...
	private void resetForm() {
		...
		toggleConvBox.getSelectionModel().select(0);
		//Run changeLabels() again because we are going to default to the default unit type too.
		this.changeLabels();
	}
